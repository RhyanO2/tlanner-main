...
export async function selectUserWorkspaces(userID) {
    return await db.select().from(Workspace).where(eq(Workspace.id_user, userID));
}
export async function selectWorkspaceById(workspaceID) {
...
export async function insertWorkspace(title, userID) {
...
export async function updateWorkspaceTitle(title, workspaceID) {
    const InsertWorkspace = await db
        .update(Workspace)
        .set({
        title: title,
